...
function loadCharts(from, to) {
  ...
  $.ajax({
      type: "POST",
      url: "form-handlers/ad_report_handler.php",
      ...
      function (data) {
          ...
          for ($j = 0; $j < Object.keys(data).length; $j++) {
              ...
              $('#charting-row' + $j).append('<div id=charting-artist' + $j + ' class=charting-artist> ' + data[$j].type + '</div>');
              $('#charting-row' + $j).append('<div id=charting-song' + $j + ' class=charting-song> ' + data[$j].name + '</div>');
...
function sortChart() {
  ...
  for (var i = 0; i < $('.charting-row').length; i++) {
      if ($('.charting-row')[i].children[4].innerHTML.trim().toLowerCase() == 'draft') {
          continue;
      }
      track = [
          $('.charting-row')[i].children[0].innerHTML.trim().toLowerCase(),
          $('.charting-row')[i].children[1].innerHTML.trim().toLowerCase()
      ];
