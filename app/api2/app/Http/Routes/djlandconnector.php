...
Route::group(array('prefix' => 'DJLandConnector'), function () {
  ...
  Route::group(array('prefix' => 'schedule'), function () {
    Route::get('/', function () {
      return DB::table('show_times')->join('shows', 'show_times.show_id', '=', 'shows.id')->select(
        'show_times.start_day',
        'show_times.start_time',
        'show_times.end_day',
        'show_times.end_time',
        'show_times.alternating',
        'show_times.show_id',
        'shows.id as show_id',
        'shows.active as active'
      )->where('shows.active', '=', 1)->get();
    });
  });
...
if (!function_exists('show')) {
  ...
  function show($id)
  {
    ...
    if (!is_countable($data[0]['show_img']) || count($data[0]['show_img']) == 0) $data[0]['show_img'] = Show::select('image as show_img')->where('id', '=', $id)->first()['show_img'];
...
if (!function_exists('playlist')) {
  ...
  function playlist($id)
  {
    ...
    $playsheet = Playsheet::select('id as playlist_id', 'show_id', 'start_time', 'end_time', 'edit_date', 'type as playlist_type', 'host as host_name', 'web_exclusive')->where('id', '=', $id)->get()->toArray();
