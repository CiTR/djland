...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::group(['prefix'=>'submissions'], function () {
        ...
        Route::put('/review', function () {
            try {
                $submission = Submissions::find(Input::get('id'));
                if ($submission -> is_trashed == 1) {
                    return "Trying to review a submission that is in the trash. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status != "unreviewed") {
                    return "Trying to review a submission that is already been reviewed. Aborting. Submission id is: " . $submission -> id;
                } else {
                    $submission -> status = "reviewed";
                    $submission -> review_comments = Input::get('review_comments');
                    $approval = Input::get('approved');
                    if ($approval == 'yes' || $approval == 'Yes' || $approval == 1) {
        ...
        Route::put('/approve', function () {
            try {
                $submission = Submissions::find(Input::get('id'));
                if ($submission -> is_trashed == 1) {
                    return "Trying to approve a review of a submission that is in the trash. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status == "unreviewed") {
                    return "Trying to appprove a review of a submission that hasn't been reviewed yet. Aborting. Submission id is: " . $submssion -> id;
                } elseif ($submission -> status != "reviewed") {
                    return "Trying to approve a review of a submission that is already been approved. Aborting. Submission id is: " . $submission -> id;
                } else {
                    $submission = Submissions::find(Input::get('id'));
        ...
        Route::put('/tag', function () {
            try {
                $submission = Submissions::find(Input::get('id'));
                if ($submission -> is_trashed == 1) {
                    return "Trying to tag a submission that is in the trash. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status == "unreviewed") {
                    return "Trying to tag a review of a submission that hasn't been reviewed yet. Aborting. Submission id is: " . $submssion -> id;
                } elseif ($submission -> status == "reviewed") {
                    return "Trying to tag a review of a submission that hasn't been approved yet. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status != "approved") {
                    return "Trying to tag a review of a submission that has already been tagged. Aborting. Submission id is: " . $submission -> id;
                } else {
                    $submission = Submissions::find(Input::get('id'));
        ...
        Route::put('/tolibrary', function () {
            try {
                $submission = Submissions::find(Input::get('id'));
                if ($submission -> is_trashed == 1) {
                    return "Trying to send a submission to the library that is in the trash. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status == "unreviewed") {
                    return "Trying to send a submission to the library that hasn't been reviewed yet. Aborting. Submission id is: " . $submssion -> id;
                } elseif ($submission -> status == "reviewed") {
                    return "Trying to send a submission to the library that hasn't been approved yet. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status == "approved") {
                    return "Trying to send a submission to the library that has not been tagged. Aborting. Submission id is: " . $submission -> id;
                } elseif ($submission -> status != "tagged") {
                    return "Trying to send a submission to the library that has already been tagged and sent to library. Aborting. Submission id is: " . $submission -> id;
                } else {
                    $submission = Submissions::find(Input::get('id'));
