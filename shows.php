...
// Option generating code
    ...
    <head>
        ...
        if(isset($_GET['action']) && $_GET['action'] == "delete") {
            echo "<h1>Show Deleted</h1>";
        }
        // SUBMITTING SHOWS -------
        else if(isset($_GET['action']) && $_GET['action'] == "submit") {
            echo "<h1>Show Added</h1>";
...
//		echo "updated: $update_q </center>";
    ...
    <div id='wrapper'>
        <p>
            ...
            <input name="showtitle" type='text' size=25 value='<?php echo $show_name ?>' style='font-size: 1.5em;' ><?php echo $show_name ?>
        </p>
        <?php if($active == 1): ?>
        ...
        <div id='access_holder'>
            <input id='member_access' class='invisible' name='member_access' value='<?php echo implode(',',$ids) ?>'>
        </div>
        ...
        <p>
            ...
            <input name='t_secondary_genre_tags' type='text' size='55' value='<?php echo $show["secondary_genre_tags"]; ?>' >
        </p>
        <hr/>
        ...
        <p style="text-decoration:underline">Social + Contact Info</p>
        <div>
            <span class="head">Service Name</span><span class="head">Address (include http/https if not email)</span><span class="head">Short Name</span><span class="head">Unlink</span>
            <?php
            $count = 1;
        ...
        <p>Notes:</p><textarea name="t_notes" cols="78" rows="14"><?php echo $notes; ?></textarea>

        <p style="float:left"><input type=submit value="Save Show"></p>
        </form>
        <?php
	...
	<?php
        echo "<FORM METHOD=\"GET\" ACTION=\"{$_SERVER['SCRIPT_NAME']}\" name=\"the_form\">\n
