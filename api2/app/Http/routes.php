...
Route::get('/nowplaying',function(){
	...
	if($specialbroadcast->first()){
		...
		//Easier to do DB:raw than using laravel query builder for this query.
...
Route::group(array('prefix'=>'resource'),function(){
	Route::put('/',function(){
		return Response::json(Resource::create());
	});
	Route::get('/',function(){
		return Resource::all();
	});
	Route::group(array('prefix'=>'{id}'),function($id = id){
		...
		Route::post('/',function($id){
			return Resource::find($id)->update( (array) Input::get()['resource']);
		});
		Route::delete('/',function($id){
