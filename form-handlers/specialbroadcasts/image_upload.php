...
if( !( isset($_POST['specialbroadcast_name']) ) ){
	//Check if Show Name Set
	$response->response = 'No Special broadcast Name Set';
	$response->ok = false;
	...
	$base_dir = $_SERVER['DOCUMENT_ROOT']."/images/specialbroadcasts/";
	if(!file_exists($base_dir)){
		mkdir($base_dir,0755);
	}

	switch($_FILES['specialbroadcastFile']['type']){
		case 'image/png':
		case 'png':
			$imageFileType = '.png';
			break;
	...
	//Create new filename (Special broadcast Name + Todays Date)
	$today = date('Y-m-d-H-i-s');
	$target_file = $target_dir.str_replace(' ','_',str_replace($strip,'',$_POST['specialbroadcast_name']))."-".$today.$imageFileType;
	$target_file_web_path = str_replace($_SERVER['DOCUMENT_ROOT'],"http://".$_SERVER['SERVER_NAME'],$target_file);
	// Check if image file is a actual image or fake image
    $check = getimagesize($_FILES["specialbroadcastFile"]["tmp_name"]);
	...
	if($uploadOk == 0){
		...
		if (move_uploaded_file($_FILES["specialbroadcastFile"]["tmp_name"], $target_file)) {
			//Attempt to set permissions to allow Apache Group to access it.
			if(chmod($target_file,0661)){
				$response->response = "The file ". basename( $_FILES["specialbroadcastFile"]["name"]). " has been uploaded.";
				$response->ok = true;
