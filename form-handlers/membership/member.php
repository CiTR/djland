    ...
    switch($request){
        case 'GET':
            ...
            if(isset($_GET['id'])){
                ...
                $query = "SELECT m.id, m.firstname, m.lastname, m.email, m.member_type FROM membership as m INNER JOIN membership_year as my ON m.id=my.member_id GROUP BY m.id";
                if(isset($_GET['order_by'])) $query.=" ORDER BY :order_by DESC";
            }
            $statement = $pdo_db->prepare($query);
            if(isset($_GET['id'])) $statement->bindValue(':id', $_GET['id']);
            if(isset($_GET['order_by'])) $statement->bindValue(':order_by',$_GET['order_by']);
            try {
                $statement->execute();
