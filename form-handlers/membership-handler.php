	...
	switch($action){
		case 'search':
			switch($type){
				case 'name':
					if($value !=  "" && $value != null){
						$query = "SELECT m.id AS id, m.firstname AS firstname, m.lastname AS lastname, m.email AS email, m.primary_phone AS phone FROM membership AS m INNER JOIN membership_years AS my ON my.member_id = m.id WHERE m.lastname LIKE '%".$value."%' OR m.firstname LIKE '%".$value."%'";
						if($year != 'all'){
							if($paid == 'both'){
						...
						$query = "SELECT m.id AS id, m.firstname AS firstname, m.lastname AS lastname, m.email AS email, m.primary_phone AS primary_phone FROM membership AS m INNER JOIN membership_years AS my ON my.member_id = m.id";
						if($year != 'all'){
							if($paid == 'both'){
