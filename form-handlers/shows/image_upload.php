...
if( !( isset($_POST['show_name']) ) ){
	...
	$response->response = 'No Show Name Set';
	$response->ok = false;
	...
	if($uploadOk == 0){
		...
		if (move_uploaded_file($_FILES["showFile"]["tmp_name"], $target_file)) {
			//Attempt to set permissions to allow Apache Group to access it.
			if(chmod($target_file,0661)){
				$response->response = "The file ". basename( $_FILES["showFile"]["name"]). " has been uploaded.";
				$response->ok = true;
