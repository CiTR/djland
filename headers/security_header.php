...
function is_paid(){
    ...
    $query = "SELECT my.paid FROM membership_years as my INNER JOIN membership as m ON my.member_id = m.id WHERE my.member_id=:member_id AND my.membership_year >= (SELECT membership_year FROM year_rollover WHERE id='1') AND my.paid='1' OR m.member_type='Lifetime' ORDER BY membership_year DESC";
